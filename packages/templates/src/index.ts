...
当前项目：${projectName}
`,
    ".ccli/harness/loop-gate.json": `${JSON.stringify(
      {
        purpose: "自动循环前的硬闸门。先让一次人工触发的任务稳定，再考虑让系统定时自己跑。",
        allowedWhenReady: [
          "低风险问题巡检",
          "整理失败摘要",
          "小范围修复",
        ...
        cadence: "只有支架健康度足够高时，才允许 30 到 60 分钟一次的小范围自动巡检。"
      },
      null,
      2
    )}\n`
...
落地分工：稳定事实放项目指南，反复流程放技能，必须阻止或必须执行的动作放确定性护栏，进度和经验放本地记忆。先让一次人工触发的任务稳定，再考虑自动循环。

自动循环闸门：只有权限护栏、验证反馈、独立审查、进度记忆和完成标准都就绪时，才允许小范围定时巡检；删除、密钥、生产、发布、主线和审查失败必须立刻停下。
