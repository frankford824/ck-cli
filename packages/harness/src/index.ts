...
export function renderHarnessMethod(): string {
  return [
    "驾驭方法：把智能体当成“模型 + 外部支架”。",
    "在 ccli 里，这套方法落成五层：",
    "1. 规范前置：项目指南、产品规则、设计契约和任务清单，让系统每次先知道边界。",
    "2. 工具收敛：每个阶段只暴露少量必要动作，先了解，再计划，再实现，再验证，再审查。",
    "3. 权限护栏：便宜、可撤回的动作自动执行；昂贵、不可逆、发布、密钥和生产动作必须中文确认。",
    "4. 反馈闭环：开发后自动验证，失败摘要先回到开发代理修复，再交给独立审查代理复核。",
    "5. 状态落盘：每个阶段写进度，踩坑写经验；下一轮先读取，不从零猜。",
    "",
    ...
    "普通老板使用时不用理解这些词，只会看到目标、进度、风险、选择和结果；技术细节留在审计记录里。",
